...
const columns = [
  ...
  {
    title: "Clothing",
    links: ["All Clothing", "Modest Wear", "Hoddies & Pullovers", "Shirts & Tops"],
  },
  {
    title: "Kids",
...
export default function Footer() {
  return (
    <footer className="bg-[--color-dark-900]">
        ...
        <div className="grid grid-cols-1 items-start gap-8 md:grid-cols-12">
          ...
          <div className="flex gap-4 md:col-span-2 md:justify-end">
            {[
              ...
              >
                <Image key={s.alt} src={s.src} alt={s.alt} width={24} height={24} />
              </Link>
            ))}
          </div>
        </div>
      ...
      <div className="border-t border-white/10">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-4 py-4 text-[--color-light-400] sm:flex-row sm:px-6 lg:px-8">
          ...
          <ul className="flex items-center gap-6 text-caption">
            {["Guides", "Terms of Sale", "Terms of Use", "Nike Privacy Policy", "Cookie Policy"].map((t) => (
              <li key={t}>
                <Link href="#">{t}</Link>
              </li>
            ))}
